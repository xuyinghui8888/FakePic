...
void algorithm::split(const std::string& in,
...
std::string algorithm::tail(const std::string& in)
{
...
std::string algorithm::firstToken(const std::string& in)
{
	if (!in.empty())
	{
		size_t token_start = in.find_first_not_of(" \t");
	...
	while (std::getline(file, curline))
	{
		...
		if (algorithm::firstToken(curline) == "f")
		{
			...
			for (int i = 0; i < int(vVerts.size()); i++)
			{
				...
				mesh_data_.pos_.push_back(CGP::float3E(vVerts[i].Position.X, vVerts[i].Position.Y, vVerts[i].Position.Z));
				mesh_data_.normal_.push_back(CGP::float3E(vVerts[i].Normal.X, vVerts[i].Normal.Y, vVerts[i].Normal.Z));
				mesh_data_.tex_cor_.push_back(CGP::float2E(vVerts[i].TextureCoordinate.X, vVerts[i].TextureCoordinate.Y));
			}
...
void Loader::genCompressedVersion()
{
