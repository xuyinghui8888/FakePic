...
using namespace CGP;
void LaplacianDeform::init(CGP::MeshCompress& input, const intVec& handle_idx, const intVec& fix_idx, const intVec& linked_vertex)
{
	int n_vertex = input.pos_.size();
	intVec combined(handle_idx.begin(), handle_idx.end());
	for (int i = 0; i < fix_idx.size(); i++)
	{
		combined.push_back(fix_idx[i]);
		fix_pos_.push_back(input.pos_[fix_idx[i]]);
	}
	int unconstrainedBegin = combined.size();
	//put others in
	intSet handle_set(combined.begin(), combined.end());
	for (int i = 0; i < n_vertex; i++)
	{
		if (!handle_set.count(i))
		{
			combined.push_back(i);
		}
	}
	handle_idx_ = handle_idx;
	fix_idx_ = fix_idx;
	link_vertex_ = linked_vertex;
	//roi= true is slow and result maybe wrong
	prepareDeform(&input.tri_[0], (int)input.tri_.size(),
		(float*)&input.pos_[0], input.pos_.size() * 3,
...
//need to fix
void LaplacianDeform::initRoi(CGP::MeshCompress& input, const intVec& handle_idx, const intVec& fix_idx)
{
	int n_vertex = input.pos_.size();
	intVec combined(handle_idx.begin(), handle_idx.end());
	for (int i = 0; i < fix_idx.size(); i++)
...
void LaplacianDeform::deform(const float3Vec& handle_pos, float3Vec& res, const LinkType& link_type)
{
	if (b_init_ == false)
	{
		LOG(ERROR) << "Laplacian Solver is not init" << std::endl;
		return;
	}
	doDeform((float*)&handle_pos[0], handle_pos.size(), (float*)&res[0], link_type);
}

void LaplacianDeform::clear()
{
	b_init_ = false;
}
...
*/
void LaplacianDeform::prepareDeform(
	const unsigned int* cells, const int nCells,
	...
	laplacianCoeffs = calcCotangentLaplacianCoeffs(
		...
		roiPositions);
		*/	
	//laplacianCoeffs = calcCotangentLaplacianCoeffs(cells, nCells, roiMap, nRoi, adj, rowBegins,roiPositions);
	...
	//s.lapMat = smooth_weight * s.lapMat;
	...
	int M = (nRoi + unconstrainedBegin + link_vertex_.size()/2) * 3;
	...
	// in order to add the second term of the energy (4) or (5), we now augment the matrix.
	{
		...
		//add weight for fix
		for (int i = unconstrainedBegin -  fix_idx_.size(); i < unconstrainedBegin; ++i) 
		{
			energyMatrixCoeffs.push_back(Triplet(i * 3 + N + 0, 3 * i + 0, fix_weight_-1));
			energyMatrixCoeffs.push_back(Triplet(i * 3 + N + 1, 3 * i + 1, fix_weight_-1));
			energyMatrixCoeffs.push_back(Triplet(i * 3 + N + 2, 3 * i + 2, fix_weight_-1));
		}
		//add weight for linked vertex has remapping select vertex order;
		int shift = N + unconstrainedBegin * 3;
		int add_link_size = shift + link_vertex_.size() / 2 * 3;
		s.b_link = Vec(add_link_size);
		s.b_link.setConstant(0);
		for (int i = 0; i < link_vertex_.size()/2; ++i)
		{
			int link_from_idx = link_vertex_[2 * i];
			int link_to_idx = link_vertex_[2 * i + 1];
			int link_from = idx_to_equation_[link_from_idx];
			int link_to = idx_to_equation_[link_to_idx];

			energyMatrixCoeffs.push_back(Triplet(i * 3 + shift + 0, 3 * link_from + 0, link_weight_));
			energyMatrixCoeffs.push_back(Triplet(i * 3 + shift + 0, 3 * link_to + 0, -link_weight_));			

			energyMatrixCoeffs.push_back(Triplet(i * 3 + shift + 1, 3 * link_from + 1, link_weight_));
			energyMatrixCoeffs.push_back(Triplet(i * 3 + shift + 1, 3 * link_to + 1, -link_weight_));

			energyMatrixCoeffs.push_back(Triplet(i * 3 + shift + 2, 3 * link_from + 2, link_weight_));
			energyMatrixCoeffs.push_back(Triplet(i * 3 + shift + 2, 3 * link_to + 2, -link_weight_));

			s.b_link[i * 3 + shift + 0] = link_weight_ * (positions[3 * link_from_idx + 0] - positions[3 * link_to_idx + 0]);
			s.b_link[i * 3 + shift + 1] = link_weight_ * (positions[3 * link_from_idx + 1] - positions[3 * link_to_idx + 1]);
			s.b_link[i * 3 + shift + 2] = link_weight_ * (positions[3 * link_from_idx + 2] - positions[3 * link_to_idx + 2]);
		}
	...
	s.b = Vec(M);
	s.roiIndices.resize(nRoi);
	SG::safeMemcpy(&s.roiIndices[0], roiIndices, sizeof(int)*nRoi);
	s.nRoi = nRoi;
	s.RSI = RSI;
}

void LaplacianDeform::doDeform(const float* newHandlePositions, int nHandlePositions, float* outPositions, const LinkType& link_type)
{
	{
		int count = 0;
		...
		for (int j = 0; j < fix_idx_.size(); j++)
		{
			s.b[count++] = fix_weight_ * fix_weight_* fix_pos_[j][0];
			s.b[count++] = fix_weight_ * fix_weight_* fix_pos_[j][1];
			s.b[count++] = fix_weight_ * fix_weight_* fix_pos_[j][2];
		}

		for (int j = 0; j < link_vertex_.size()/2; j++)
		{
			s.b[count++] = 0;
			s.b[count++] = 0;
			s.b[count++] = 0;
		...
		if (link_type == LinkType::KEEP_DIS)
		{
			s.b = s.b + s.b_link;
		}
	...
	Vec minimizerSolution;
	{
		...
		LOG(INFO) << "y:" << y.sum() << std::endl;
		LOG(INFO) << "minimizerSolution:" << minimizerSolution.sum() << std::endl;
	}
	...
	if (s.RSI) {
		...
		LOG(INFO) << "normalizedSolution:" << normalizedSolution << std::endl;
