...
void MTCNN::loadPRONet(const string& proto_model_dir)
{
...
void MTCNN::init(const CGP::JsonData& init_json)
{
	std::string PNet_loc = init_json.root_ + init_json.mtcnn_PNet_;
	std::string RNet_loc = init_json.root_ + init_json.mtcnn_RNet_;
	std::string ONet_loc = init_json.root_ + init_json.mtcnn_ONet_;
	PNet_ = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(PNet_loc.c_str()));
	RNet_ = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(RNet_loc.c_str()));
	ONet_ = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(ONet_loc.c_str()));
	init_network();
	setIsMaxFace(init_json.is_max_face_);
	setMinFace(init_json.face_min_size_);
}


void MTCNN::init(const string& proto_model_dir)
{
	loadPRONet(proto_model_dir);
	init_network();
}
