#ifndef _DEEP3D_REC_H_
#define _DEEP3D_REC_H_
...
#pragma omp parallel for
	  for (int y = 0; y < h; y++)
	  {
		  for (int x = 0; x < w; x++)
		  {
			  for (int c = 0; c < 3; c++)
			  {
				  input_img[3*(y*width_ + x)+c] = float((img_rgb_float.at<cv::Vec3f>(y, x))(c));
			  }			  
		  }
	  }
	  ...
	  intVec num_out = { 80,64,80,3,27,2,1 };
	  cstrVec name_out = { "515", "521", "527", "533", "539", "545", "551" };
