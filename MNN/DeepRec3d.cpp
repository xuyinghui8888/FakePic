...
void Deep3D::init(const CGP::JsonData& init_json)
{
...
void Deep3D::init(const CGP::JsonData& init_json, const CGP::cstr model_relative_path)
{
	//LOG(INFO) << height_ << std::endl;
	height_ = init_json.deep3d_input_;
	width_ = init_json.deep3d_input_;
...
void Deep3D::print_tensor(const Tensor &mat, const int n)
{
...
int Deep3D::get_max_preds(const vector<vector<float> >&heatmaps, const int width, vector<pair<int, int> > &kps, vector<float> &max_vals){
