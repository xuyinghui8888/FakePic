...
namespace bvh11
{
    namespace internal
    {
		...
		inline Eigen::Vector3d read_offset_blender(const std::vector<std::string>& tokens)
		{
			assert(tokens.size() == 4 && tokens[0] == "OFFSET");

			Eigen::Quaternionf rq_y_z = Eigen::Quaternionf::FromTwoVectors(Eigen::Vector3f(0,0,1), Eigen::Vector3f(0, 1, 0));
			Eigen::Matrix3f rm_y_z = rq_y_z.toRotationMatrix();
			//std::cout << rm_y_z * Eigen::Vector3f(1, 0, 0) << std::endl;
			//std::cout << rm_y_z * Eigen::Vector3f(0, 1, 0) << std::endl;
			//std::cout << rm_y_z * Eigen::Vector3f(0, 0, 1) << std::endl;
			const double offset_x = std::stod(tokens[1]);
			const double offset_y = -std::stod(tokens[3]);
			const double offset_z = std::stod(tokens[2]);
			return Eigen::Vector3d(offset_x, offset_y, offset_z);
		}
    }
	...
	void BvhObject::ReadGuijieFile(const std::string& file_path, const double scale)
	{
		// Open the input file
		std::ifstream ifs(file_path);
		assert(ifs.is_open() && "Failed to open the input file.");
		...
		[&]() -> void
		{
			...
			while (std::getline(ifs, line))
			{
				...
				else if (tokens[0] == "ROOT" || tokens[0] == "JOINT")
				{
					...
					// Read the joint name
					std::string joint_name = tokens[1];

					std::string::size_type mo = joint_name.find("Guijie_");
					if (mo != std::string::npos)
					{
						//std::cout << "joint_name: " << joint_name << std::endl;
						//std::cout << "mo: " << mo << std::endl;
						joint_name = joint_name.substr(7);
						//std::cout << "joint_name: " << joint_name << std::endl;
					}
				...
				else if (tokens[0] == "OFFSET")
				{
					assert(tokens.size() == 4);
					const std::shared_ptr<Joint> current_joint = stack.back();
					//current_joint->offset() = scale * internal::read_offset_blender(tokens);
					current_joint->offset() = scale * internal::read_offset(tokens);
				}
				// Read a channel list
				else if (tokens[0] == "CHANNELS")
...
#if 0
				//swap
				if (channels_.size() % 3 != 0)
				{
					std::cout << "error in channels size." << std::endl;
				}
				else
				{
					//Eigen::Quaternionf rq_y_z = Eigen::Quaternionf::FromTwoVectors(Eigen::Vector3f(0, 0, 1), Eigen::Vector3f(0, 1, 0));
					...
					int size = channels_.size() / 3;
					for (int iter_swap = 1; iter_swap < size; iter_swap++)
					{					
						//Quaternion qBefore = qZ * qY * qX;
						double x = motion_(frame_index, iter_swap * 3 + 0);
						double y = motion_(frame_index, iter_swap * 3 + 1);
						double z = motion_(frame_index, iter_swap * 3 + 2);

						Eigen::AngleAxisd rollAngle((z*M_PI) / 180, Eigen::Vector3d::UnitZ());
						Eigen::AngleAxisd yawAngle((y*M_PI) / 180, Eigen::Vector3d::UnitY());
						Eigen::AngleAxisd pitchAngle((x*M_PI) / 180, Eigen::Vector3d::UnitX());
						Eigen::Quaterniond qBefore = pitchAngle * yawAngle * rollAngle;
						Eigen::Quaterniond rq_y_z = Eigen::Quaterniond::FromTwoVectors(Eigen::Vector3d(0, 0, 1), Eigen::Vector3d(0, 1, 0));
			      	    Eigen::Matrix3d rx = (qBefore).toRotationMatrix();
						Eigen::Vector3d ea = rx.eulerAngles(0, 2, 1);
						motion_(frame_index, iter_swap * 3 + 0) = ea(0)*180.0/M_PI;
						motion_(frame_index, iter_swap * 3 + 1) = ea(1)*180.0 / M_PI;
						motion_(frame_index, iter_swap * 3 + 2) = ea(2)*180.0 / M_PI;
					}
				}		
#endif			
			}
	...
	void BvhObject::WriteGuijieJointSubHierarchy(std::ofstream& ofs, std::shared_ptr<const Joint> joint, int depth) const
	{
		auto indent_creation = [](int depth) -> std::string
		{
			std::string tabs = "";
		...
		for (auto child : joint->children())
		{
			WriteGuijieJointSubHierarchy(ofs, child, depth + 1);
		}
	...
	void BvhObject::WriteGuijieFile(const std::string& file_path) 
	{
		std::list<std::shared_ptr<bvh11::Joint> > temp = root_joint_->children_;
		// must be one
		//std::cout << temp.size() << std::endl;

		root_joint_->children_ = (temp.front())->children_;
		for (auto i : root_joint_->children_)
		{
			i->parent_ = root_joint_;
		}
		root_joint_->name_ = temp.front()->name();
		//std::cout << root_joint_->end_site_ << std::endl;
		//std::cout << root_joint_->offset_ << std::endl;
		root_joint_->offset_ = temp.front()->offset_;
		// Eigen format
		const Eigen::IOFormat motion_format(Eigen::StreamPrecision, Eigen::DontAlignCols, " ", "", "", "\n", "", "");
		...
		// Hierarch
		ofs << "HIERARCHY" << "\n";
		WriteGuijieJointSubHierarchy(ofs, root_joint_, 0);
		...
		for (int y = 0; y < motion_.rows(); y++)
		{
			//ofs << motion_(y, 0) << " " << motion_(y, 1) << " " << motion_(y, 2) << " ";
			ofs << root_joint_->offset_(0) << " " << root_joint_->offset_(1) << " " << root_joint_->offset_(2) << " ";
			for (int x = 6; x < motion_.cols(); x++)
			{
				ofs << motion_(y, x) << " ";
			}
			ofs << std::endl;
		...
		std::cout << file_path << " frames: " << motion_.rows() << std::endl;
		//ofs << motion_.format(motion_format);
	}
