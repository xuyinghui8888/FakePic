...
namespace bvh11
{
    ...
    public:
        ...
        BvhObject(const std::string& file_path, const double scale = 1.0, bool b_guijie = false)
        {
			if (!b_guijie)
			{
			...
			{
				ReadGuijieFile(file_path, scale);
			}
		...
		/// \param file_path Path to the output BVH file.
		void WriteGuijieFile(const std::string& file_path);
    ...
    private:
		...
		void ReadGuijieFile(const std::string& file_path, const double scale = 1.0);
		...
		void WriteGuijieJointSubHierarchy(std::ofstream& ofs, std::shared_ptr<const Joint> joint, int depth) const;
    };
    ...
    public:
		...
		std::list<std::shared_ptr<Joint>>& varChildren() { return children_; }
        const std::list<int>& associated_channels_indices() const { return associated_channels_indices_; }
        

		void setChild(std::list<std::shared_ptr<Joint> > children) { children_ = children; };

		void setParent(std::shared_ptr<Joint> parent) { parent_ = parent; };
