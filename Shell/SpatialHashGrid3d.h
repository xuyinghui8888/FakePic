...
class CSpatialHashGrid3D
{
public:
  CSpatialHashGrid3D(unsigned int ndiv, double center[3], double half_width);
  ...
  void GetCellCand(const double p[3], std::vector<unsigned int>& aIndTriCand) const;
...
private:
  ...
  void SetData(int i, int j, int k,  std::vector<unsigned int>& aIndTriCand) const;
...
private:		
  ...
  double org_[3];
  double width_;
  CGP::doubleVec width_vec_ = { 0,0,0 };
