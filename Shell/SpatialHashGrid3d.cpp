...
CSpatialHashGrid3D::CSpatialHashGrid3D(unsigned int ndiv, double center[3], double half_width)
{
	this->ndiv_ = ndiv;
...
void CSpatialHashGrid3D::GetCellCand(
	const double p[3], std::vector<unsigned int>& aIndTriCand) const
{
	aIndTriCand.clear();
	if (p[0] < org_[0]) return;
	if (p[1] < org_[1]) return;
	if (p[2] < org_[2]) return;
	if (p[0] > org_[0] + width_vec_[0]) return;
	if (p[1] > org_[1] + width_vec_[1]) return;
	if (p[2] > org_[2] + width_vec_[2]) return;
	int ip[3];	this->GetIndex(p, ip);
	SetData(ip[0], ip[1], ip[2], aIndTriCand);
}

void CSpatialHashGrid3D::AddTri(int itri, double p0[3], double p1[3], double p2[3])
{
...
void CSpatialHashGrid3D::AddData(unsigned int i, unsigned int j, unsigned int k, int idata)
{
	assert( i < ndiv_ );
	assert( j < ndiv_ );
	assert( k < ndiv_ );	
...
void CSpatialHashGrid3D::BuildOutFlg()
{	
	aFlgOut.clear();
...
bool CSpatialHashGrid3D::IsOut(double p[3]) const
{
...
void CSpatialHashGrid3D::SetData
(int i, int j, int k,  
 std::vector<unsigned int>& aIndTriCand) const
{	
...
void CSpatialHashGrid3D::Find_NearestTriCand
(double p[3], 
...
void CSpatialHashGrid3D::Find_Intersec_Layer_X
(unsigned int i,
 const double p0[2], 
 const double p1[2],
 std::vector<unsigned int>& aIndTriCand)
...
void CSpatialHashGrid3D::Find_Intersec_Layer_Y
(unsigned int j,  // y
...
void CSpatialHashGrid3D::Find_Intersec_Layer_Z
(unsigned int k,  // z
...
void CSpatialHashGrid3D::Find_IntersecTriCand
(const double _p[3], 
 const double d[3],
 std::vector<unsigned int>& aIndTriCand)
{	
